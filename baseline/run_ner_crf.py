        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    predict(args, model, tokenizer, prefix="checkpoint-{}".format(global_step))
                    logger.info("Saving prediction to %s", output_dir+"checkpoint-{}".format(global_step))
        logger.info("\n")
...
def main():
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            results.update(result) # the function of `update` is to add the `result` to the dictionary
