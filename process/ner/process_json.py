...
sys.path.append(".")
import pandas as pd
from process.text.process_text import process_text
...
def process_json_text(json_objects):
...
def sort_entities(json_object):
    # 函数功能：对实体进行排序, 按照"研究问题"、"研究方法"、"研究材料"、"评估度量"、"研究成果"的顺序
...
def sort_entities(json_object):
    # 函数功能：对 JSON 对象中的字段按照 "text_id", "text", "entities" 的顺序进行排序
...
def split_dataset(json_objects: list):
    ...
    val_json =  json_objects[int(length * 0.8):int(length * 0.9)]
    test_json =  json_objects[int(length * 0.9):]
...
def save_to_json(json_objects, save_to_file):
    # 函数功能：保存到文件
    # 将 JSON 对象转换为单行字符串
...
def process_json(json_objects, source_tsv_file):
    # 函数功能：处理 JSON 对象
    
    # 过滤不需要的标签
    json_objects = filter_entities(json_objects)
    # 处理文本：去除多余空格、替换中文标点符号、去除特殊字符
    json_objects = process_json_text(json_objects)
    # 生成索引
    ...
    json_objects = add_fields(json_objects, source_tsv_file=source_tsv_file)
...
if __name__ == '__main__':
    ...
    json_data_dir = "dataset/ner"
    save_dir = os.path.join('dataset', 'ner')
    
    source_tsv_file = os.path.join(raw_data_dir, "csl_camera_readly_filter_cleaned.tsv")
    json_objects = json.load(open(os.path.join(json_data_dir, "csl_camera_annotated_1_10000.json"), 
                                  'r', encoding='utf-8'))
    
    # 处理数据
    json_objects = process_json(json_objects, source_tsv_file)
    
    save_to_json(json_objects, save_to_file=os.path.join(save_dir, "csl_camera_annotated_1_10000_result_fileter_format_cleaned.json"))
    
    # 分割数据集
    train_json, val_json, test_json = split_dataset(json_objects)
    save_files = ['train.json', 'dev.json', 'test.json']
    
    for idx, json_objects in enumerate([train_json, val_json, test_json]):
        save_to_json(json_objects, save_to_file=os.path.join(save_dir, save_files[idx]))
